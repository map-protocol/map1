MAP v1 — Specification v1.1
Canon: MAP1 (v1)
Status: v1.1 (frozen)

This version is v1.0 + BOOLEAN and INTEGER type additions.
v1.0 shipped with four types (STRING, BYTES, LIST, MAP) and mapped
JSON booleans to STRING "true"/"false", rejected all JSON numbers.
Community feedback identified two problems:
  (a) boolean-string collision: true and "true" produced the same MID
  (b) rejecting all numbers was too restrictive for common use cases

v1.1 adds BOOLEAN (tag 0x05) and INTEGER (tag 0x06) to the canonical
model and updates the JSON-STRICT adapter accordingly. No changes to
CANON_HDR, existing MCF tags (0x01-0x04), key ordering, projection
semantics, MID format, or error precedence.

All v1.0 MIDs that did not involve JSON booleans remain identical
under v1.1. MIDs involving JSON booleans will change because booleans
now encode as type 0x05 instead of STRING.
...
1) A canonical model (STRING, BYTES, LIST, MAP, BOOLEAN, INTEGER).
...
- FULL-MINIMUM: reserved for future version (see Appendix D).
...
[Reserved for future version — see Appendix D.]
...
- BOOLEAN: a value that is exactly true or false.
...
- true  encodes as: 0x05 0x01
- false encodes as: 0x05 0x00
- No other payload_byte values are valid. Decoders MUST reject any
  BOOLEAN payload_byte other than 0x00 or 0x01 with ERR_CANON_MCF.

INTEGER encoding:
- The value is encoded as a signed 64-bit integer in big-endian byte order.
- Always exactly 9 bytes total: 1 tag byte + 8 value bytes.
- Valid range: -9,223,372,036,854,775,808 to 9,223,372,036,854,775,807.
- The integer 0 encodes as 0x06 followed by 8 zero bytes.
- Out-of-range values MUST be rejected with ERR_TYPE at the adapter layer.

Constraints:
- MAP keys MUST be encoded as STRING values. (Keys cannot be BOOLEAN, INTEGER, BYTES, LIST, or MAP.)
...
- For any container C (MAP or LIST) that contains a child value V:
    ...
    - If V is a scalar (STRING, BYTES, BOOLEAN, or INTEGER), it does not increase depth.
...
ERR_TYPE        - unsupported type in adapter layer (e.g., JSON null, JSON float)
...
- JSON number  -> INTEGER if the number is a valid signed 64-bit integer; otherwise ERR_TYPE

8.2.1 JSON Number Rules (normative)
JSON numbers are accepted only if ALL of the following conditions are met:
(a) The JSON number token does not contain a decimal point character '.'.
(b) The JSON number token does not contain an exponent indicator 'e' or 'E'.
(c) The numeric value, when parsed as a signed integer, is within the range
...
- -1       -> INTEGER (accepted)
- 0        -> INTEGER (accepted)
- 3.14     -> ERR_TYPE (decimal point)
- 1.0      -> ERR_TYPE (decimal point, even though value is mathematically integral)
- 1e5      -> ERR_TYPE (exponent)
- 1E5      -> ERR_TYPE (exponent)
- 9223372036854775807  -> INTEGER (INT64_MAX, accepted)
- -9223372036854775808 -> INTEGER (INT64_MIN, accepted)
- 9223372036854775808  -> ERR_TYPE (exceeds INT64_MAX)
- -9223372036854775809 -> ERR_TYPE (below INT64_MIN)

The token-level check (conditions a and b) is intentional. The adapter
inspects the raw JSON number token string, not the parsed numeric value.
This prevents silent coercion (e.g., 1.0 being treated as integer 1).
...
- INTEGER overflow handling: implementations MUST validate integer range before encoding. Languages with
  arbitrary-precision integers (Python, Ruby) MUST explicitly range-check against INT64 bounds. Languages
  with fixed-width integers (Go, Rust) get this for free from the type system but MUST handle JSON parsing
  carefully to avoid silent truncation (especially JavaScript, where JSON.parse converts all numbers to
  IEEE 754 doubles, silently losing precision for integers above 2^53).
...
v1.1
- Added BOOLEAN type (tag 0x05). JSON booleans now encode as BOOLEAN
  instead of STRING "true"/"false". This eliminates the type collision
  where true and "true" produced the same MID.
- Added INTEGER type (tag 0x06). JSON numbers that are valid signed
  64-bit integers (no decimal point, no exponent, within int64 range)
  are now accepted. Floats, exponent notation, and out-of-range integers
  remain rejected with ERR_TYPE.
- Updated JSON-STRICT type mapping (§8.2) and added JSON Number Rules (§8.2.1).
- Updated canonical model type list (§3.1) and type tag table (§3.2).
- Updated fast-path validation (§3.7) to include BOOLEAN payload validation.
- Updated depth definition (§4) to list BOOLEAN and INTEGER as scalars.
- Added security note on integer overflow handling across languages (§9).
- No changes to CANON_HDR, existing type tags (0x01-0x04), key ordering,
  projection semantics, MID prefix format, error codes, or error precedence.

v1.0
- Initial frozen release. Four-type canonical model (STRING, BYTES, LIST, MAP).
- JSON booleans mapped to STRING, all JSON numbers rejected.
...
A6) Version prefix stability
MAP's identity function is versioned by prefix (map1:). The addition of BOOLEAN and INTEGER types in v1.1 does not change
the map1: prefix. Future type additions that preserve the canonical framing do not alter the map1: identity class. A new
prefix (e.g., map2:) would only be introduced for changes that break canonical encoding compatibility.
...
Layer 1: Adapter profile (JSON-STRICT)
   ...
   - reject BOM
   - map booleans to BOOLEAN
   - map integers (in range, no decimal/exponent) to INTEGER
   - reject floats, nulls
   |
   v
Layer 2: Canonical model (STRING / BYTES / LIST / MAP / BOOLEAN / INTEGER)
   |
   v
...
These vectors add coverage only; they MUST NOT require rebaselining
existing golden outputs (except for vectors involving JSON booleans,
which now encode differently under v1.1).
...
- JSON true MUST produce a different MID than JSON string "true".
- JSON false MUST produce a different MID than JSON string "false".
- [true] MUST produce a different MID than ["true"].

C14. INTEGER type distinction
- JSON number 42 MUST produce a different MID than JSON string "42".
- JSON number 0 MUST produce a different MID than JSON string "0".

C15. INTEGER boundary
- INT64_MAX (9223372036854775807) MUST be accepted.
- INT64_MIN (-9223372036854775808) MUST be accepted.
- INT64_MAX + 1 MUST be rejected with ERR_TYPE.
- INT64_MIN - 1 MUST be rejected with ERR_TYPE.

C16. Float rejection
- JSON numbers with decimal points (3.14, 1.0) MUST be rejected with ERR_TYPE.
- JSON numbers with exponents (1e5, 1E5) MUST be rejected with ERR_TYPE.

C17. Mixed-type containers
- A LIST containing STRING, BOOLEAN, and INTEGER values MUST produce a deterministic MID.
- A MAP containing STRING, BOOLEAN, and INTEGER values MUST produce a deterministic MID.

APPENDIX D — FULL-MINIMUM (RESERVED FOR FUTURE VERSION)

The FULL-MINIMUM projection mode is deferred. The following text
